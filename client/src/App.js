...
function SecondaryAdminUpdateDuty() {
  ...
  React.useEffect(() => {
    ...
    if (attendance && date) {
      const att = attendance.find(a => a.date === date && a.dutySchedule === dutySchedule);
      setComingTime(att?.comingTime || '');
      setFinishingTime(att?.finishingTime || '');
    }
  }, [date, attendance, dutySchedule]);
  ...
  const handleDeleteRecord = async (record) => {
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`${API_URL}/api/users/${selectedId}/attendance/${record.date}?dutySchedule=${encodeURIComponent(record.dutySchedule || '')}` , {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` },
      });
      const data = await res.json();
  ...
  const handleEditSubmit = async (values) => {
    try {
      ...
      const res = await fetch(`${API_URL}/api/users/${selectedId}/attendance/${editingRecord.date}`, {
        ...
        body: JSON.stringify({ ...values, originalDutySchedule: editingRecord.dutySchedule || '' }),
      });
      const data = await res.json();
      if (res.ok) {
  ...
  const attendanceColumns = [
    ...
    { title: 'Schedule', dataIndex: 'dutySchedule', key: 'dutySchedule' },
  ...
  const handleQrScan = async (scannedText) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setAttendance(data.attendance || []);
        const today = dayjs().format('YYYY-MM-DD');
        const todayRecord = (data.attendance || []).find(a => a.date === today && a.dutySchedule === dutySchedule);
        ...
        <Content style={{ margin: 24, background: '#fff', borderRadius: 8, minHeight: 400, padding: 24 }}>
          ...
          {attendance && attendance.length > 0 && (
            <div style={{ maxWidth: 700, margin: '40px auto 0 auto' }}>
              ...
              <Table columns={attendanceColumns} dataSource={attendance} rowKey={(r) => `${r.date}-${r.dutySchedule || 'NA'}`} />
            </div>
          )}
          ...
          {attendance && attendance.length > 0 && (
            <div style={{ maxWidth: 800, margin: '40px auto 0 auto' }}>
              ...
              <Table 
                ...
                rowKey={(r) => `${r.date}-${r.dutySchedule || 'NA'}`}
                scroll={{ x: 400 }}
                pagination={{
