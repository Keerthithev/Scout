...
router.put('/:id/duty', authMiddleware, requireAnyRole(['super', 'secondary']), async (req, res) => {
  try {
    ...
    if (!user) return res.status(404).json({ message: 'User not found' });
    let updated = false;
    // Find attendance for the date and schedule (treat schedule as part of key)
    const att = user.attendance.find(a => a.date === date && (dutySchedule ? a.dutySchedule === dutySchedule : !a.dutySchedule));
    if (att) {
      if (comingTime) att.comingTime = comingTime;
      if (finishingTime) att.finishingTime = finishingTime;
      if (dutySchedule !== undefined) att.dutySchedule = dutySchedule || '';
      updated = true;
    } else {
      user.attendance.push({ date, dutySchedule: dutySchedule || '', comingTime: comingTime || '', finishingTime: finishingTime || '' });
      updated = true;
    }
    await user.save();
...
router.put('/:id/attendance/:date', authMiddleware, requireAnyRole(['super', 'secondary']), async (req, res) => {
  try {
    ...
    // Identify the record by originalDutySchedule if provided, otherwise by current dutySchedule
    const identifierSchedule = req.body.originalDutySchedule ?? dutySchedule;

    // Find the attendance record for the specific date and schedule
    const attendanceRecord = user.attendance.find(a => a.date === date && (identifierSchedule ? a.dutySchedule === identifierSchedule : !a.dutySchedule));
    if (!attendanceRecord) {
      return res.status(404).json({ message: 'Attendance record not found for this date and schedule' });
    }
...
router.delete('/:id/attendance/:date', authMiddleware, requireAnyRole(['super', 'secondary']), async (req, res) => {
  try {
    const { date } = req.params;
    const { dutySchedule } = req.query;

    const user = await User.findById(req.params.id);
    if (!user) return res.status(404).json({ message: 'User not found' });

    // Find and remove the attendance record for the specific date and schedule
    const attendanceIndex = user.attendance.findIndex(a => a.date === date && (dutySchedule ? a.dutySchedule === dutySchedule : !a.dutySchedule));
